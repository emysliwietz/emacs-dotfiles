...
(defcustom symon-refresh-rate 1
  "refresh rate of symon display. *set this option BEFORE
  enabling `symon-mode'.*"
  :group 'symon)

(defcustom symon-delay 1
...
(defcustom symon-sparkline-height (if (string= (shell-command-to-string "hostname | tr -d '\n'") "astaroth") 22 11)
  "height of sparklines."
  :group 'symon)

(defcustom symon-sparkline-width (if (string= (shell-command-to-string "hostname | tr -d '\n'") "astaroth") 80 40)
  "width of sparklines."
  :group 'symon)
...
(defcustom symon-sparkline-thickness (if (string= (shell-command-to-string "hostname | tr -d '\n'") "astaroth") 4 2)
  "line width of sparklines."
  :group 'symon)

(defcustom symon-sparkline-type 'boxed
  "type of sparklines."
  :group 'symon)
...
(define-symon-monitor symon-linux-battery-monitor
  :index "BAT:" :unit "%" :sparkline f
  :fetch (when battery-status-function
           (read (cdr (assoc ?p (funcall battery-status-function))))))

(define-symon-monitor symon-corona-monitor
  :display
;  (symon-linux--read-lines
					;   "/home/user/.cache/statusbar/corona" 'read '(""))
  (shell-command-to-string "cat ~/.cache/statusbar/corona | tr -d '\n'")
  )
...
(define-symon-monitor symon-current-time-monitor
  :display (shell-command-to-string "date_format | tr -d '\n'"))

(define-symon-monitor symon-delim-monitor
  :display "|")
